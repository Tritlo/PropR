...
to run it on the infinitely looping `gcd'`. Note that the fixes are currently
limited to replacing the expression itself due to the limitations of the
`makeHoley` function, but this will be fixed in a future version.
