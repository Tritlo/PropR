...
findPropLocations
  desc@ProbDesc
    ...
    let nze
          | evaluatedLeavesOnly = map leavesOnly non_zero_evals
          | otherwise = non_zero_evals
        reconstituted = reconstitute used_props nze
    liftIO $ logStr DEBUG "Reconstituted:"
    liftIO $ mapM_ (logOut DEBUG) reconstituted
