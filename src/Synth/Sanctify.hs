...
sanctifyGRHSs (GRHSs x grhss lbs) =
  map (\(l, g') -> (l, GRHSs x g' lbs)) (applToEach sanctifyGRHS grhss)
    ++ slb
  where
    slb = map (second (GRHSs x grhss)) $ sanctifyLocalBinds lbs
sanctifyGRHSs _ = []
